...
- 新增 Ren'Py / TyranoScript 离线端到端 corpus（#424 P6）：`tests/test_engine_offline_corpus.py` 冻结两个 fixture 的 candidate / occurrence / classification 计数与 `inventory_digest` / `coverage_digest`，并验证 coverage package / review template；Ren'Py 的 `build -> check -> apply` 产品路径由既有 golden batch 套件覆盖，Tyrano 保持 adapter 库层 fixture corpus。真实 Ren'Py SDK 运行时的语言切换 / fallback 未在 CI 验证，边界已记录在 `docs/engine_adapter.md`。
