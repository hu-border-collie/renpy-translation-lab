...
- #487 新增 Ren'Py 字体字形覆盖只读 spike：`font_coverage.py` + `scripts/font_coverage_report.py` 解析静态 `font` 引用与 sfnt cmap，输出 `checked / missing / unknown`、稳定 `font.*` reason code 与脱敏证据；动态表达式 / FontGroup / 越界路径进入 unknown，不启动游戏、不执行游戏 Python、不替换字体。新增可再分发 Noto 子集 fixture（OFL-1.1）、正反例测试与 doctor / preflight 接入建议。
