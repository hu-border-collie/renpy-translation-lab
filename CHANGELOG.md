...
- #431 真实 smoke 准备：扩充 [Provider / 执行策略 / 最终审校 Smoke Matrix](docs/provider_smoke_matrix.md)，补齐直连 `openai_compatible`（目录 / `profiles-probe` / 小范围 `sync-start`）与 `final-review-run-sync` 的最短路径、调用预算、稳定机器码、脱敏记录模板和待执行清单；离线合同测试与真实 Provider 证据继续分开记录。
- #487 新增 Ren'Py 字体字形覆盖只读 spike：`font_coverage.py` + `scripts/font_coverage_report.py` 解析静态 `font` 引用与 sfnt cmap，输出 `checked / missing / unavailable / unknown`、稳定 `font.*` reason code 与脱敏证据；动态表达式 / FontGroup / 越界路径 / 空文本样本进入 unknown，不启动游戏、不执行游戏 Python、不替换字体。新增可再分发 Noto 子集 fixture（OFL-1.1）、正反例测试与 doctor / preflight 接入建议。
- #427 S1 新增普通译文逐条审校索引与人工决定日志：`review_index.py` 从 `export-revision-corpus` 的 corpus + 现有 `quality_findings.jsonl`（可选 `translation_records.jsonl`）构建可重建 `review_index.jsonl` / manifest / Markdown，重复 occurrence 通过 `occurrence_id` 保持独立，未匹配 finding 与孤儿决定进入 diagnostics。独立 `review_decisions.jsonl` 支持 `open / ignored / resolved` 生命周期、reviewer 与 binding provenance；重建时 source/target/context/evidence 任一 digest 变化派生 `needs_recheck`，历史保留。新增 `review-index-build` / `review-index-status` / `review-decisions-import` / `review-decisions-export` 离线 CLI 与 JSON envelope；S1 不新增第二翻译库、不触碰 quality-ack/reuse/写回授权，GUI 订正页属 S2。
...
- 新增四类组合验证矩阵文档 [Provider / 执行策略 Smoke Matrix](docs/provider_smoke_matrix.md)，覆盖 Gemini Sync、LiteLLM 内置 Provider、自定义 OpenAI-compatible Provider、Gemini Batch 的命令、离线自动化覆盖和记录口径；`profiles-show --output json --output-file` 可作为脱敏路由诊断摘要（#431 后续已扩充直连 adapter 与最终审校 sync）。
...
- #422 `apply` 新增翻译差分文件树导出：P1 `--export-only` 只导出本次字节确有变化的完整文件，不修改游戏树或翻译进度；P2 `--export-dir` 把同一份已验证渲染结果同时写回工作区并导出，workspace/export/receipt 共用一个 `apply_export` journal，提交后才幂等推进 progress、RAG、latest（条件推进）、manifest 与 receipt。严格回滚持久化 `rolling_back`/`rolled_back` 阶段，回滚中断后可重放；事务外修改 fail closed。complete receipt 重放会重新复核两边输出与 workspace target 父路径身份（symlink/junction/reparse），删/改/增或路径身份改变返回 `APPLY_EXPORT_OUTPUT_CHANGED`；RAG 返回 error、store 不可用或配置关闭时，`pending_steps=["rag"]` 是权威待办并保持 `APPLY_EXPORT_STATE_PENDING`（无静默 waive），重新可执行且成功后才 complete；latest 的所有物理写入口（CLI、`save_manifest(update_latest=True)`、GUI `ProjectState.remember_latest_manifest_path`）统一走 `atomic_io` 共享锁服务并在同一 `<latest>.lock` 内做读-比较-写；被其它操作推进时保留新值并报告 `retained_newer`，latest 锁不做自动 stale 抢占，owner 已死/未知/记录损坏都等待 timeout 并结构化失败，遗弃锁只能人工确认无 writer 后删除（generic 配置/同步锁的抢占竞态已在 #474 用 kernel 级进程锁移除，见本节修复条目）。POSIX 下 workspace 根以 `O_NOFOLLOW` 打开并与路径 stat 比对身份，根或父目录在复核前后被替换会稳定返回 `APPLY_EXPORT_OUTPUT_CHANGED`；Windows 窄 TOCTOU 窗口仍按已知限制记录。pending_steps 以 receipt/manifest 并集为权威，任一侧 complete 都不能抹除；receipt 写入失败时 `last_error` 为 best-effort 并报告 pending；P1 `--export-only` 拒绝未决 P2 journal/receipt，不执行 P2 副作用。CLI 机器 envelope、GUI「导出…」目录选择/预览/执行入口、错误码与文档同步更新。
...
- 最终审校 `final-review-build` / `final-review-resume` / `final-review-ingest-results` 改为取冻结的 final_review 阶段路由模型：campaign manifest 的 `model` / `batch_model`、resume 重建请求与结果解析使用同一模型；无 `model_routing` 的旧 campaign 回退到 manifest 记录模型，legacy 入口仍要求 `gemini_batch` 策略（#344 ④）。
- 文档口径同步：同步执行与 Gemini Batch 明确为并列的一等入口，不再把同步描述为仅小范围 / 补译；Sync 仍默认只生成可审查预览，Batch 用于远程排队与成本折扣场景。
...
- 新增离线 `coverage-status` 与 `coverage-review-import`（#424 P6 Slice 3a）：前者只读展示 coverage 状态、分类/reason 计数、gate/review 状态与未解决候选 locator；后者校验并原子安装 review 到 `<game_root>/translation_context/coverage_review.json`，pending/stale/invalid 结构化拒绝，支持 `--dry-run`。
- doctor 新增 `engine_status` 块（#424 P6 Slice 3b）：复用同一次 live adapter 扫描，以稳定 `engine.*` reason code 报告 adapter capabilities/protocol、locator 与 candidate schema、最新 `logs/project_snapshots/*/project_snapshot.json` 的 schema/engine/adapter 兼容性、coverage catalog provenance/freshness，以及 `latest_manifest` 的 TranslationPlan fingerprint 与 source identity。`error` 级问题使该块 `blocked`、抑制 workflow state 并让 doctor envelope 返回 `status=blocked`；Durable Sync 与非 Batch strategy 委派给各自 run-store/plan 合同；不调用模型、不写盘。
- GUI 环境检查接入 coverage/review 展示与操作（#424 P6 Slice 3c）：doctor 的 `coverage` 块新增未解决候选 locator 摘要；环境检查摘要显示文本覆盖状态、分类、review/gate 与未解决候选定位，并提供「重新扫描文本覆盖」与「导入核对结果」按钮。导入复用 `run_coverage_review_import` 同一 schema/freshness/policy 合同，pending/stale/invalid 以结构化 code 拒绝；导入在后台线程执行，切换项目后旧结果不刷新当前项目，不修改 `.rpy`。
- GUI 新增引擎 / 快照 / 复用候选只读展示（#424 P6）：上下文库提供「引擎与快照…」「复用候选…」入口。对话框展示当前 engine / adapter capabilities、`logs/project_snapshots/*/project_snapshot.json` 清单，调用现有 `reconcile_project_snapshots` 显示版本 diff（disposition、匹配依据、置信度、base/target locator、候选与证据），并调用 `load_reuse_candidates` 展示已有复用候选的类别、状态、歧义目标、译文来源与证据。导出快照、构建候选、导入决策与结果导出仍只在 CLI，GUI 不重新实现匹配、不写回。
- `translator_config.example.json` 与 GUI 设置 schema / runtime reader 对账（#424 P6）：补齐 prepare 自定义命令与 launcher、batch retry、safety_settings、macro_setting、keyword_extraction、revision 等已受支持的键；新增回归测试确保示例覆盖全部 `ADVANCED_SETTING_FIELDS` 且示例值通过同一 `normalize_for_write` 校验。
- 新增 Ren'Py / TyranoScript 离线端到端 corpus（#424 P6）：`tests/test_engine_offline_corpus.py` 冻结两个 fixture 的 candidate / occurrence / classification 计数，并断言同一 checkout 内 `inventory_digest` / `coverage_digest` 稳定（不跨 OS 固定 digest，Git CRLF 检出会合法改变源字节），同时验证 coverage package / review template；Ren'Py 的 `build -> check -> apply` 产品路径由既有 golden batch 套件覆盖，Tyrano 保持 adapter 库层 fixture corpus。真实 Ren'Py SDK 运行时的语言切换 / fallback 未在 CI 验证，边界已记录在 `docs/engine_adapter.md`。
- coverage review 硬门禁接入（#424 P6 Slice 2）：独立 review 约定路径为 `<game_root>/translation_context/coverage_review.json`，新增 `evaluate_coverage_gate` 同时校验 live coverage 与 review freshness / policy / unresolved findings。`project-analysis-publish` 在写入 published 前要求 gate confirmed（`--force` 不绕过）；Final Review readiness 在 gate 未确认时拒绝，即使 `pending=0`；doctor 的 `coverage` 块新增 review status / policy / unresolved findings / gate 字段。
- 修复 #486：最终审校 `final-review-ingest-results` 在归一化前按 response schema 校验 root / finding 必填字段、类型与枚举，拒绝未知 item 引用和同一 item 的完全重复 finding（同 item 的不同合法问题仍并存）。失败 unit 写入稳定分类 `failed_to_parse_model_json` / `missing_findings` / `schema` / `duplicate_item`，detail 只含索引、字段与本地 item 身份、不回显 provider 原文；`final-review-status` 与 ingest 的机器输出新增 `failure_reason_counts`，GUI 状态行用同一分类显示失败原因。`issues` / `id` / `type` / `detail` / `suggestion` 软别名继续兼容（规范字段优先），合法空 findings 成功，unit 级 targeted resume 与写回门禁不变；completion receipt 仍未设为强制合同。
