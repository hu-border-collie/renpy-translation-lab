...
- #422 `apply` 新增翻译差分文件树导出：P1 `--export-only` 只导出本次字节确有变化的完整文件，不修改游戏树或翻译进度；P2 `--export-dir` 把同一份已验证渲染结果同时写回工作区并导出，workspace/export/receipt 共用一个 `apply_export` journal，提交后才幂等推进 progress、RAG、latest（条件推进）、manifest 与 receipt。严格回滚持久化 `rolling_back`/`rolled_back` 阶段，回滚中断后可重放；事务外修改 fail closed。complete receipt 重放会重新复核两边输出，删/改/增返回 `APPLY_EXPORT_OUTPUT_CHANGED`；RAG 返回 error 时保持 `APPLY_EXPORT_STATE_PENDING` 并允许重试；latest 被其它操作推进时保留新值并报告 `retained_newer`；P1 `--export-only` 拒绝未决 P2 journal/receipt，不执行 P2 副作用。CLI 机器 envelope、GUI「导出…」目录选择/预览/执行入口、错误码与文档同步更新。
