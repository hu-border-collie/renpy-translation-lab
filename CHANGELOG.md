...
- #422 `apply` 新增翻译差分文件树导出：P1 `--export-only` 只导出本次字节确有变化的完整文件，不修改游戏树或翻译进度；P2 `--export-dir` 把同一份已验证渲染结果同时写回工作区并导出，workspace/export/receipt 共用一个 `apply_export` journal，提交后才幂等推进 progress、RAG、`applied_at`、next split 与 latest。prepared journal 整体回滚，committed journal + pending receipt 在下次 apply 幂等补记；恢复对事务外修改 fail closed。CLI 机器 envelope、GUI「导出…」目录选择/预览/执行入口、错误码与文档同步更新。
