...
当前 ingest 的固定 fixture 基线发现两个现行误接受边界：finding 缺 response schema 必填字段、完全重复 finding；另单列一个未来 completion receipt 数量不符的假设性探针。它们尚未进入生产 parser；本阶段也不启用宽松 JSON repair。现状对比、候选稳定码、unit 级 targeted resume 合约和 receipt 暂缓结论见 [Final Review 结果失败分类 fixture spike](archive/final_review_result_failure_spike.md)。
