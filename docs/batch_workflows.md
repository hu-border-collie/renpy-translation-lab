...
- P2 导出收据记录 request/文件摘要、workspace 摘要、`apply_identity`、`latest_previous_value` / `latest_target` 与 pending/complete 阶段。重复运行同一 `apply_identity` + 导出根时，所有幂等成功出口都会复核 receipt、workspace target 父路径身份（symlink/junction/reparse）、受管 export 树与 workspace 字节；删/改/增输出或路径身份改变返回 `APPLY_EXPORT_OUTPUT_CHANGED`，不复用旧成功摘要。RAG 步骤返回 error、store 不可用或当前配置关闭 RAG 时，文件保持已提交，但 receipt/manifest 的 `pending_steps=["rag"]` 是权威待办，保持 `APPLY_EXPORT_STATE_PENDING` / `rag_status=blocked_disabled|blocked_no_jobs|error`；配置关闭不会把待办抹成 complete，也没有静默 waive 路径，重新可执行后成功才 complete。latest 游标在共享 `exclusive_file_lock` 内读-比较-写；只有当前值仍等于 receipt 记录的前值时才推进，已被其它合法操作推进时保留新值并记录 `retained_newer` 诊断，不回退。
