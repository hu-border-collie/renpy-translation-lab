...
- P2 导出收据记录 request/文件摘要、workspace 摘要、`apply_identity`、`latest_previous_value` / `latest_target` 与 pending/complete 阶段。重复运行同一 `apply_identity` + 导出根时，所有幂等成功出口都会复核 receipt、受管 export 树与 workspace 字节；删/改/增输出返回 `APPLY_EXPORT_OUTPUT_CHANGED`，不复用旧成功摘要。RAG 步骤返回 error 时文件保持已提交，但 receipt/manifest 保持 `pending_steps=["rag"]` 并返回 `APPLY_EXPORT_STATE_PENDING`；下一次 apply 重跑 RAG，成功后才 complete。latest 游标只有当前值仍等于 receipt 记录的前值时才推进；已被其它合法操作推进时保留新值并记录 `retained_newer` 诊断，不回退。
