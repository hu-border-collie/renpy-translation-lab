...
- P2 的 `apply --export-dir <PATH>` 把同一份已验证渲染结果同时提交到工作区实际 TL 源文件和导出树：workspace 文件、export payload 与 `apply_export_record.json` 共用一个 `apply_export` journal；全部文件提交后才幂等推进 progress、RAG、`applied_at`、next split 与 latest manifest。跨磁盘集合不声称瞬时全局原子性，目标是失败可恢复、状态可解释：prepared journal 会整体回滚；committed journal 与 pending receipt 会在下次 apply 时补记状态；事务外修改会让 recovery fail closed，绝不覆盖。`--force` 不绕过 stale check、源快照、结构阻断、plan 绑定或导出目录冲突。
- P2 导出收据记录 request/文件摘要、workspace 摘要、`apply_identity` 与 pending/complete 状态；重复运行同一 `apply_identity` + 导出根会幂等返回 `applied_and_exported`，不同导出根则拒绝。未完成的状态推进通过 receipt 的 progress/RAG/latest 计划幂等补记，不会重复推进或误报未发生。
- 机器 envelope 会在 `result.apply` 中区分 `mode=export-only` 的 `status=exported / no-op`，以及 `mode=apply-export` 的 `status=applied_and_exported / no-op`；并给出 `export_root`、`record_path`、`exported_files`、`applied_files`、`actual_applied_files`、`applied_lines`、`recovery_state` 与 `state_advancement_status`。失败或待恢复使用稳定的 `EXPORT_ONLY_*` / `APPLY_EXPORT_*` 错误码，错误详情包含 `journal_path`、`record_path`、`export_root` 与 `recovery_state`。
