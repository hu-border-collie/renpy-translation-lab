...
检查后可把当前 `quality_findings.jsonl` 导出为单文件 HTML 体检报告。报告内置规则、严重程度、确认状态与全文筛选，可离线打开或打印；它只读取当前 manifest 和 findings，不修改译文、确认状态或写回门禁：

```powershell
python gemini_translate_batch.py quality-report logs/batch_jobs/<package>/manifest.json
python gemini_translate_batch.py quality-report logs/batch_jobs/<package>/manifest.json --html review/quality.html
```

默认输出为 findings 同目录下的 `quality_report.html`。报告内容可能包含游戏原文、译文和文件名，分享前应按私有项目资料处理。
...
结构化输出当前覆盖核心命令（`doctor / build / submit / status / download / check / apply / quality-ack / quality-unack / quality-report`）、版本资产/复用命令、revision / keyword / final-review 工作流命令；完整清单以 `capabilities` 的 `machine_output` 标记为准，其它命令继续以各自帮助和落盘 JSON/JSONL 为准。
...
- `quality-report` 读取 manifest 当前指向的 `quality_findings.jsonl`，原子写入自包含 HTML；`--html` 可指定输出路径。HTML 会转义 finding 文本、不加载外部资源，并保留当前 warning 确认状态作为只读快照。
...
- P1 的 `apply --export-only <PATH>` 复用同一套 check、写回闸门、源复核、adapter plan 与 `render_writeback_plan`，但只把最终字节确有变化的完整文件复制到导出树；导出根按 manifest 绑定的游戏根与已验证真实源路径计算，不硬编码语言目录。它不会修改游戏文件、进度、RAG、`applied_at`、拆分游标或 latest manifest；同 package 存在未决 P2 journal 或 pending receipt 时会结构化拒绝（`APPLY_EXPORT_RECOVERY_REQUIRED`），不会替 P2 回滚文件或补记状态，也不会创建请求的导出目录。
- 首次 `--export-only` 要求目标目录不存在或为空；重复请求只有在同一导出回执、整棵受管树、源快照和门禁仍匹配时才幂等成功。非空目录、回执/树冲突、路径逃逸/重叠根、符号链接/联接、源漂移、stale check 或非 `allow` 门禁都会拒绝；`--force` 不绕过这些检查。`--export-only` 不支持 Durable Sync、revision 或其它非 translation manifest。
- P2 的 `apply --export-dir <PATH>` 把同一份已验证渲染结果同时提交到工作区实际 TL 源文件和导出树：workspace 文件、export payload 与 `apply_export_record.json` 共用一个 `apply_export` journal；全部文件提交后才幂等推进 progress、RAG、latest（条件推进）、manifest 与 receipt。跨磁盘集合不声称瞬时全局原子性，目标是失败可恢复、状态可解释：`prepared` journal 会进入可重放的 `rolling_back` / `rolled_back` 回滚阶段；`committed` journal 与 pending receipt 会在下次 apply 时补记状态；事务外修改会让 recovery fail closed，绝不覆盖。
- P2 导出收据记录 request/文件摘要、workspace 摘要、`apply_identity`、`latest_previous_value` / `latest_target` 与 pending/complete 阶段。重复运行同一 `apply_identity` + 导出根时，所有幂等成功出口都会复核 receipt、workspace target 父路径身份（symlink/junction/reparse）、受管 export 树与 workspace 字节；删/改/增输出或路径身份改变返回 `APPLY_EXPORT_OUTPUT_CHANGED`，不复用旧成功摘要。RAG 步骤返回 error、store 不可用或当前配置关闭 RAG 时，文件保持已提交，但 receipt/manifest 的 `pending_steps=["rag"]` 是权威待办，保持 `APPLY_EXPORT_STATE_PENDING` / `rag_status=blocked_disabled|blocked_no_jobs|error`；配置关闭不会把待办抹成 complete，也没有静默 waive 路径，重新可执行后成功才 complete。latest 游标的所有物理写入口（CLI、`save_manifest(update_latest=True)`、GUI `ProjectState.remember_latest_manifest_path`）都走 `atomic_io` 共享锁服务并对同一 `<latest>.lock` 做读-比较-写；只有当前值仍等于 receipt 记录的前值时才推进，已被其它合法操作推进时保留新值并记录 `retained_newer` 诊断，不回退。latest 锁不做任何自动 stale 抢占：即使 owner PID 可证明已死或记录损坏，也等待到 timeout 并结构化失败；遗弃锁只能人工确认无 writer 后删除。generic `config_store` / `sync_run_store` / usage ledger 锁已改用 kernel 级进程锁（POSIX `flock` / Windows `LockFile`，见 #474）：进程死亡由操作系统自动释放，不再有按年龄抢占或 read-then-unlink 竞态；锁文件释放后保留，不得删除正在使用的锁文件，旧版与新版 writer 不得混跑。pending_steps 在 receipt/manifest 的并集是权威：任一侧 complete 都不能抹除；receipt 写失败时 `last_error` 为 best-effort，CLI 报 `receipt_updated=false` 并保持 pending。
...
最终审校的模型与执行方式取冻结的 `final_review` 阶段路由：campaign manifest 记录 `execution_strategy`（`gemini_batch` / `sync`）与 `model_routing` 快照；无 `model_routing` 的旧 campaign 回退到 manifest 记录的模型并按 `gemini_batch` 处理。v1 配置可显式把 `routes.final_review.strategy` 配为 `sync`；旧配置迁移仍生成 `gemini_batch`。
...
该实验只覆盖**同步路由**下的变体，不能提供 Sync 与真实 Gemini Batch 之间的成本、排队时间或恢复可靠性对比；新用户 / 新项目默认值的最终决策需要另行实测（#457）。
