...
- [#426 Ren'Py coverage block 归因报告](plans/renpy_coverage_block_attribution.md)：只读脚本与原创 fixture 的 unknown / unsupported / parse_error 归因、漏扫检查和后续拆单草案；P1 跨行字符串 inventory（#460）、source marker 级联（#461）、TL 缺 marker 分类策略（#462）与 digest canonicalization（#463）已实现。
