...
- [#426 Ren'Py coverage block 归因报告](plans/renpy_coverage_block_attribution.md)：只读脚本与原创 fixture 的 unknown / unsupported / parse_error 归因、漏扫检查和后续拆单草案；#460–#464 五条 follow-up 已全部实现（跨行字符串 inventory、source marker 级联、TL 缺 marker 分类策略、quoted comment / 孤儿 `old` 复核、digest canonicalization）。
