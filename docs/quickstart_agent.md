...
P1/P2 还支持把本次实际发生字节变化的完整文件导出为文件树。`--export-only` 只导出，不修改游戏树或翻译进度；`--export-dir` 把同一份渲染结果同时写回工作区并导出，使用一个可恢复 journal：
...
`result.apply.mode=export-only` 时读取 `status=exported` 或 `no-op`、`export_root`、`record_path`、`exported_files`、`applied_files=0` 和 `recovery_state`。`result.apply.mode=apply-export` 时读取 `status=applied_and_exported` 或 `no-op`、`exported_files`、`applied_files`、`actual_applied_files`、`applied_lines`、`recovery_state`、`pending_steps`、`latest_cursor` 与 `state_advancement_status`。目标目录首次必须不存在或为空；重复调用只接受匹配回执与完整受管树，且任何幂等成功出口都会重新复核输出，删/改/增输出返回 `APPLY_EXPORT_OUTPUT_CHANGED`。RAG 返回 error、store 不可用或配置关闭时，文件已提交但收到 `APPLY_EXPORT_STATE_PENDING`；`pending_steps=["rag"]` 是权威待办，配置关闭不会抹除，下一次可执行时才会重跑并 complete（没有静默 waive）。latest 游标在共享锁内读-比较-写：只在前值仍匹配时条件推进，其它操作已推进时保留新值并报告 `retained_newer`；latest 锁不做自动 stale 抢占，即使 owner 已死或记录损坏也会超时并返回 `APPLY_EXPORT_STATE_PENDING`，遗弃锁只能人工确认无 writer 后删除，避免两个恢复者互相删除锁。workspace target 父路径被换成 symlink/junction/reparse 时，即使字节未变也会返回 `APPLY_EXPORT_OUTPUT_CHANGED`。`--export-only` 遇到未决 P2 journal/pending receipt 时结构化拒绝，不执行 P2 恢复或状态补记。两个选项互斥，都不接受 Durable Sync、revision 或其它非 translation manifest；`--force` 不绕过 stale check、源快照、结构阻断、plan 绑定、输出复核或导出目录冲突。
...
python gemini_translate_batch.py final-review-run-sync <manifest> --output json
...
`final-review-build` 按 `routes.final_review.strategy` 选择执行方式：`gemini_batch` 继续
`final-review-resume → submit → status → download → final-review-ingest-results`；
`sync` 使用 `final-review-run-sync` 直接逐 unit 执行并写入 findings，重复运行即按
input/context digest 续跑（`--force` 重跑全部）。`final-review-resume` /
`final-review-ingest-results` 只服务 Batch campaign，遇到 sync package 返回稳定机器码，
不会静默切换执行方式。
...
python gemini_translate_batch.py profiles-list-models --profile <PROFILE_ID> --output json
```
...
- `final-review-run-sync` 的 `status` 为 `completed / no_work / dry_run`；`result` 暴露
  `run_count / skip_count / deferred_count / done_delta / failed_delta / finding_count`
  与 `campaign_status`。unit 级失败按稳定 `sync_<category>` 前缀记录后继续，系统性失败
  （鉴权 / 缺少依赖 / 不支持能力）返回 `FINAL_REVIEW_SYNC_ABORTED` 与已落盘的部分结果。
...
`preview-revisions / export-keywords / final-review-status / final-review-export / final-review-resume / final-review-run-sync / final-review-ingest-results / final-review-create-revisions`
...
python gemini_translate_batch.py translate-preflight --strategy sync --profile <PROFILE_ID>
python gemini_translate_batch.py translate-preflight --strategy sync --output json
```
