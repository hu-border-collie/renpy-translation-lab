...
终端、GUI 与 manifest 都会显示首次/最终完整率、定点重试次数和未解决项。同步翻译的最终有效数只统计同时通过模型合同和本地 adapter 校验、可以进入安全 preview 的条目；关键词没有预期候选数量，因此明确显示完整请求块数与请求块完整率，而不把 chunk 数描述成候选条目数。重试后仍不完整时任务标为 `partial`，同时保留通过合同的结果：同步翻译只为这些结果生成安全 preview；同步订正和关键词报告也会明确显示部分完成。`partial` 不是质量认可，写回前仍须人工审查。
