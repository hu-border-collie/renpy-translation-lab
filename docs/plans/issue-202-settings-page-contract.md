...
> Phase D 已将「模型」「项目」「上下文」「高级」「外观」页迁到独立 `SettingsPage`；其余 4 页与保存/dirty/离开保护
...
- **补建页面被 preserve/restore 覆盖**：Phase B 已修。只保留已加载页面的快照，且恢复只回写
  快照中存在的 advanced 键。
- **页面不可独立构造**：部分收敛。LiteLLM、「模型」、「项目」、「上下文」、「高级」与「外观」页已是独立 `SettingsPage`；其余 4 页仍经
...
- 其余 4 页仍以 `LegacySettingsPageAdapter` 运行。
...
Phase D（进行中，模型页、项目页、上下文页、高级页与外观页）：
...
- 测试：`tests.test_settings_models_page`、`tests.test_settings_project_page`、
  ...
  `tests.test_settings_appearance_page`（独立构造）与既有
