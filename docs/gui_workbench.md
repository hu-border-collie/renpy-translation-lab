...
本文档说明可选 PySide6 图形工作台能做什么、怎么装、以及和 CLI 的边界。GUI 仍在持续迭代（issue #42）。
...
**第一版已覆盖**：项目选择与配置、环境检查、一键 Batch 翻译与长任务恢复、写回安全保护、主题、Batch 上下文预建、高级诊断页（任务上下文 / 命令 / Manifest + 原始日志）。仓库自动化测试也会验证「未安装图形依赖时 CLI 仍可运行」。

**有意延后或拆 follow-up**：在真实游戏工程上的端到端手测记录、repair/retry 图形入口、安装包与多项目管理等。