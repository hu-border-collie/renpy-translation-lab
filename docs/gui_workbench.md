...
- **需处理**：禁用写回。可先查看问题清单，必要时生成「补译包」并预览；预览确认后可点击写回页「**继续补译**」编排 `submit → status → download → check → merge-retry → check` 全流程；云端排队中可再次点击继续，或在工作台使用「继续翻译」从 `status` 恢复。修完问题后点击「**重新检查**」对父任务记录单独执行 `check`，更新写回摘要与安全闸门；显示「可写回」后才能写入项目。写回页仍保留「补救命令」入口作高级回退。
...
- 还没有完整多项目管理。
- 还没有完整可视化 diff 编辑器。
- repair 仍作为独立高级 CLI 流程，不包装成同一个「需处理」解锁路径。
