...
> **状态（2026-07）**：Epic #157 信息架构 **P0a–P3（#158–#166）已合并**；EmptyState 摘要修复见 [PR #177](https://github.com/hu-border-collie/renpy-translation-lab/pull/177)。本文是**现行用户/开发手册**。设计过程稿见历史文档 [gui_ia_redesign.md](gui_ia_redesign.md)。`app.py` 进一步页面化解耦见 [#176](https://github.com/hu-border-collie/renpy-translation-lab/issues/176)。
...
| **工作台左导航** | 五个固定任务入口（见下）。切换时**保留各任务页会话**（进度摘要、写回 manifest、关键词 candidates、订正可写回态、批量阶段索引等）。运行中导航禁用。 |
...
**壳与导航**：全局项目栏、左导航五任务页、共享状态三栏（环境检查/进度/写回）、批量进度区高级工具（试跑/拆分）、页会话往返、工作台日志抽屉、环境检查、写回安全与「问题处理」折叠、设置分区（含项目级上下文开关）、主题、上下文库预建、「诊断与工具」。
...
**有意延后**：安装包、更完整的 per-game 全量配置档、按 [#176](https://github.com/hu-border-collie/renpy-translation-lab/issues/176) 推进的页面化与 `app.py` 解耦。
