...
- **工作区**：浏览 `games_registry.json` 中的全部项目，扫描/刷新总表、同步 `GAMES.md`、编辑项目详情，并切换当前 `game_root`。切换成功后会自动打开「项目」分区。此分区的操作即时写入 registry，不使用底部「保存设置」。
- **账户**：读取 / 保存 `api_keys.json`；环境变量 Key 只读提示。
- **项目**：`game_root`、术语表、翻译目录、include filters，以及准备流程的 source game、Ren'Py SDK、Python、launcher 和自定义命令。通常建议先在「工作区」选择项目，再在这里微调路径。
