...
- **外部初译实验**：无任务时命令参考包含 `work-export`；选中外部工作包后按材料读取、成果接收、状态查询、`check` → `work-preview` → `work-apply` 的顺序提供命令。可复用现有质量报告查看结果；首版不增加工作包编辑器或第二套审校入口。合同及恢复诊断见 [外部初译工作包](external_translation_work.md)。
