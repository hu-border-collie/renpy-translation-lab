...
**任务选择框架（进行中）**：工作台已提供「任务类型 + 子任务」联动选择与按子任务切换的 UI 外壳；Batch 翻译与预建库已可用，其余子任务待 #85 / #83 / #84 接入各自 workflow。

**有意延后或拆 follow-up**：同步翻译模式（#85）、关键词提取模式（#83）、完整订正模式（#84）、retry 图形化编排、repair 独立 GUI 模式、安装包、多项目管理、更完整的多游戏端到端手测矩阵等。
