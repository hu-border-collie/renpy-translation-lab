...
- **需处理**：禁用写回。可先查看问题清单，必要时生成「补译包」并预览；修完问题后点击写回页「**重新检查**」对当前任务记录单独执行 `check`，更新写回摘要与安全闸门；显示「可写回」后才能写入项目。写回页还提供「补救命令」入口，诊断页命令参考会给出补译相关可复制命令。
...
- 还没有完整多项目管理。
- 还没有完整可视化 diff 编辑器。
- 补译包目前提供写回页按钮、预览对话框和诊断页命令参考；`submit` 之后的补译全流程仍主要靠诊断页命令；repair 仍作为独立高级 CLI 流程，不包装成同一个「需处理」解锁路径。
