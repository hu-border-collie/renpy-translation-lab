...
- 顶部：当前游戏 work 目录选择与路径显示（也可先选项目根目录）。
- **任务类型 + 子任务**（两列下拉，联动）：
  - **翻译**：Batch 翻译、同步翻译（均已实现）
  - **分析与准备**：Batch 关键词、同步关键词、预建 RAG 库、预建原文索引
  - **维护**：订正、同步订正（均已实现）
  切换子任务会更新主按钮文案、进度 Tab 与写回说明；未实现的子任务会提示暂时使用命令行。预建库操作从配置页移至此处，配置页仍保留开关。
...
- **Batch 上下文**：RAG、原文索引、build 时自动补建等开关。启用后需先保存配置，再到工作台「分析与准备」运行预建子任务。
...
- **CLI 命令**：按当前 manifest 生成可复制的手动命令（`doctor`、`submit`、`status`、`download`、`check`、`apply` 等）；当最近一次 `check=warn` 时，也会补出 `build-retry` / retry `submit` / `status` / `download` / `merge-retry` / `check` 参考命令。
...
在工作台选择「翻译 · 同步翻译」后点击「开始翻译」，GUI 会调用 `gemini_translate.py`（无额外参数），配置仍来自 `translator_config.json` 的 `sync.*` 段。同步模式会按脚本现有行为直接写回 `.rpy`；写回 Tab 仅作说明，不提供 Batch 的 `check/apply` 闸门。运行前需已配置 API Key。

### 订正（Batch 与同步）

在工作台选择「维护 · 订正」后点击「生成订正预览」，GUI 会编排 Batch 订正流程：
...
- GUI「订正写回」页在预览显示有可写回项（`Recoverable revision items > 0`）且 manifest 尚未设置 `revision_applied_at` 时，才应启用写回；已写回过的 manifest 不会再次启用普通 apply。
- **不要在唯一原项目上直接整批写回**；请先在副本或备份上验证预览报告（`revision_preview.jsonl` / `revision_preview.md`）。

## 关键词提取

关键词模式只生成术语与剧情候选报告，**不会修改游戏 `.rpy` 文件**。写回 Tab 仅展示结果说明，不提供翻译写回按钮。

### Batch 关键词

在工作台选择「分析与准备 · Batch 关键词」后点击「提取关键词」，GUI 会编排：

```text
build-keywords -> submit -> status -> download -> export-keywords
```

如果 Batch 任务仍在处理中，GUI 会停在等待状态，用户稍后可以点击「继续提取」。恢复逻辑会读取 latest manifest，并校验它是当前项目的 `keyword_extraction` 任务；如果 latest manifest 属于其他项目或其他模式，会拒绝继续。

如果 build 已生成 package 但还没有 job，恢复会从 submit 继续，而不是错误地直接跑 status。

`export-keywords` 完成后，界面会摘要候选数量与 JSONL / Markdown 报告路径，完整路径可在诊断页复制。

### 同步关键词

在工作台选择「分析与准备 · 同步关键词」后点击「提取关键词」，GUI 会调用 `gemini_translate_batch.py sync-keywords`（无额外参数）。适合小范围即时生成报告；不支持从 manifest 恢复。运行前需已配置 API Key。
...
```text
保存 Batch 上下文开关 -> 工作台选「分析与准备」-> 预建 RAG / 原文索引 -> 切回「翻译 · Batch」开始翻译
```
...
- `warn`：禁用写回，引导用户查看问题；写回页提供「补救命令」入口，诊断页命令参考会给出 retry 的最小可复制命令，处理后仍需重新 `check` 到 `safe`。
...
- 还没有完整多项目管理。
- 还没有完整可视化 diff 编辑器。
- retry 目前提供诊断页命令参考和写回页跳转入口；repair 仍作为独立高级 CLI 流程，不包装成同一个 `check=warn` 解锁路径。
...
**真实项目烟测记录**：2026-06-19 使用一部约 3,300 待译行的真实 Ren'Py 项目跑通 GUI 基础主路径：预处理、原文索引预建、`build -> submit -> status`、排队后继续、`download -> check`、`safe` 写回和游戏启动验证。该批次影响 6 个文件、约 3,300 处译文行；首轮 `check` 返回 12 个 `warn` 项时 GUI 正确禁用写回，问题经 CLI / 人工修正并重新 `check=safe` 后再写回。

**任务选择框架（进行中）**：工作台已提供「任务类型 + 子任务」联动选择与按子任务切换的 UI 外壳；Batch 翻译、同步翻译、关键词提取、订正 workflow 模块与预建库已可用。

**有意延后或拆 follow-up**：订正写回按钮的完整 GUI 接线（`app.py`）、retry 图形化编排、repair 独立 GUI 模式、安装包、多项目管理、更完整的多游戏端到端手测矩阵等。
