...
**当前 GUI 兼容限制**：上述两个按钮仍只对旧 `version=1` translation manifest 启用，而当前 `build` 生成 `version=2` / `manifest_version=2`。因此新建 v2 任务需在「诊断与运行日志 · 命令参考」复制并使用 CLI `probe` / `split`；这是 GUI 包装层限制，不影响 CLI 处理当前 manifest。高级选项仍为 `probe --limit/--offset/--api-key-index` 与 `split --max-chunks/--max-items/--display-name-prefix`。详见 [上下文系统](context_systems.md)。
...
- GUI 的拆分 / 试跑按钮暂只接受旧 `version=1` manifest；当前 v2 任务请使用命令参考中的 CLI。
