...
页眉下方为**全局项目栏**（P0b / #159）：任意顶层 Tab 均可见当前 `game_root`；**切换项目**打开设置 → 工作区总表，**指定本地目录…** 走文件夹选择（切换前会处理未保存设置）。任务运行中二者禁用。
...
- 项目路径以**全局项目栏**为准（工作台不再重复主「选目录」叙事）。浏览工作区内全部 registry 项目请 **设置 → 工作区**（详见 [工作区项目总表](games_registry.md)）。
- **左侧任务导航**（P1a / #160）：批量翻译 · 同步翻译 · 关键词/术语 · 订正 · 上下文库。多模式页（关键词/订正/上下文库）另有「模式」下拉（批量|同步 或 记忆库|原文索引）。切换导航会**保留各模式会话**（workflow / 写回 manifest / 完成快照等），任务运行中导航禁用。
- 说明行仍更新主按钮文案、进度 Tab 与写回 Tab 标签。
...
- **批量翻译三阶段**（P1b / #161）：阶段条 **准备 → 执行 → 结果** 切换内容（批量模式下隐藏下方平级 Tab 栏，避免双导航）：
  - **准备**（其他模式仍称「环境检查」）：`collect_doctor_report` 摘要与记忆库/原文索引状态；用于解锁翻译流程。
  - **执行**（其他模式为进度 Tab）：时间线、进度与任务事实；开始/继续/停止。
  - **结果**（其他模式为写回/说明）：检查摘要；主按钮「写回翻译」；**问题处理**折叠（需处理时自动展开）。
  阶段索引随模式 session 一并保留（默认执行）。
...
工具栏提供「刷新上下文」「翻译 A/B 对比」与「清空日志」（以及关键词场景下的「合并到 glossary」快捷入口）。切换到诊断 Tab 时会重新读取最近任务记录；流程进行中会优先展示当前活动的记录。
