...
上下文库页中的 **项目分析** 已按产物生命周期提供主操作：首次为「开始分析」，有结构后为「生成项目摘要」，已发布时为「更新项目摘要」，脚本变化、结构缺失或上次构建失败时为「重新分析」。首次或重新分析会先询问是否导入当前项目最近的剧情概要，然后由统一任务进度串联导入、结构构建与摘要生成；跳过导入也必须明确选择。即使合法空项目没有 label/route，也会根据真实结构产物进入后续流程。

生成完成后自动打开 **审查与启用**：可查看完整 draft / published 差异和全文，搜索 chunk / label / route，查看 evidence item、来源文件与行跨度，并打开来源文件或复制 `path:line`。**实际注入预览**使用翻译运行时相同的发布、fingerprint、开关和字符上限门禁，显示最终会进入 prompt 的截断后正文或不能注入的原因。
