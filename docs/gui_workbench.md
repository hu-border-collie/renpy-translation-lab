...
| 区域 | 作用 |
|------|------|
| **全局项目栏** | 任意顶层 Tab 可见当前 `game_root`；**切换项目**打开设置 → 工作区总表；**指定本地目录…** 走文件夹选择（切换前处理未保存设置）。任务运行中二者禁用。 |
...
- **准备**：环境检查摘要与记忆库/原文索引状态；用于解锁翻译流程。尚未检查时显示空状态 CTA「运行环境检查」。
- **执行**：时间线、进度与任务事实；开始 / **继续**（无可恢复任务时禁用并提示） / 停止；**高级工具**（试跑样本、拆分翻译包）；有拆分时「提交剩余包」。尚无进度时显示空状态 CTA。
...
**壳与导航**：全局项目栏、左导航五任务页、批量三阶段、执行区高级工具（试跑/拆分）、页会话往返、工作台日志抽屉、环境检查、写回安全与「问题处理」折叠、设置分区（含项目级上下文开关）、主题、上下文库预建、「诊断与工具」。
