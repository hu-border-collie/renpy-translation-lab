...
**真实项目烟测记录**：2026-06-19 使用一部约 3,300 待译行的真实 Ren'Py 项目跑通 GUI 基础主路径：预处理、原文索引预建、`build -> submit -> status`、排队后继续、`download -> check`、可写回状态下写回和游戏启动验证。该批次影响 6 个文件、约 3,300 处译文行；首轮检查为「需处理」时 GUI 正确禁用写回，问题经 CLI / 人工修正并重新检查为「可写回」后再写回。
