...
上下文库页中的 **项目分析** 已按产物生命周期提供主操作：首次为「开始分析」（静态构建结构），有结构后为「生成项目摘要」，已发布时为「更新项目摘要」，脚本变化、结构缺失或上次构建失败时为「重新构建」。即使合法空项目没有 label/route，也会根据真实结构产物进入后续流程。审查、启用到翻译、停止用于翻译会依据草稿、发布和 fingerprint 状态启用或禁用。启用开关、是否用于翻译、模型和思考等级集中在「设置 → 上下文 → 项目剧情分析」；其余长度、并发等参数仍在高级设置。详见 [上下文系统 · Project Analysis](context_systems.md#project-analysis项目分析)。
