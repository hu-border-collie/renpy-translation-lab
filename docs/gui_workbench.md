...
- **外部初译实验**：无任务时命令参考包含 `work-export`；选中外部工作包后提供材料读取、成果接收、剩余项/冲突查询、`check`、绑定预览及写回/恢复命令。可复用现有质量报告查看结果；首版不增加工作包编辑器或第二套审校入口。合同及操作顺序见 [外部初译工作包](external_translation_work.md)。
