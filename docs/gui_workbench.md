...
> **信息架构（Epic #157）**：**P0a–P3（#158–#166）已合并**，环境检查 EmptyState 的摘要重叠也已由 [PR #177](https://github.com/hu-border-collie/renpy-translation-lab/pull/177) 修复。本文描述当前界面；真实页面化与 `app.py` 解耦见后续 [#176](https://github.com/hu-border-collie/renpy-translation-lab/issues/176)。
...
| **诊断页** | 任务上下文、命令参考、任务记录 + 原始日志；**翻译 A/B 对比**等仍从这里启动并放大本页日志。试跑 / 拆分见工作台批量 · 执行 · 高级工具。 |
...
- **上下文**：批量 RAG 记忆库、原文索引、build 时自动补建、上下文库保存位置；以及 **同步 RAG / 同步·批量剧情记忆主开关**（P2b 单源，仅此处编辑主开关）。启用后需先保存设置，再到工作台 **上下文库** 页预建。
- **外观**：浅色 / 深色 / 跟随系统。切换主题会立即预览，保存设置后才写入 `translator_config.json`。
- **高级**：翻译吞吐、retry、Batch safety settings、术语/风格、关键词提取、订正、RAG / 原文索引 / 剧情记忆的**检索参数**与 store 路径（主开关见「上下文」）。数值字段会按 CLI 语义校验，路径类字段可留空表示使用默认路径；列表字段支持逐行填写，命令和 safety settings 支持 JSON。

底部 **重新加载 / 恢复推荐值 / 保存设置** 只作用于 `translator_config.json` 相关字段；**工作区** 总表与切换项目即时写 registry，不依赖「保存设置」。
...
- **从本页启动的工具**（翻译 A/B 对比）：仍在诊断页放大下方日志区域。
- **试跑 / 拆分**在工作台 **批量翻译 · 执行 · 高级工具**（P2a / #164），输出进工作台日志抽屉。
...
- A/B 对比仍在诊断页；拆分 / 试跑已在批量 · 执行 · 高级工具（P2a / #164）。

## 已覆盖范围

**GUI 工作台（#42 + Epic #157 P0–P3）**：全局项目栏、左导航五任务页、批量三阶段、执行区高级工具（试跑/拆分）、页会话往返、工作台日志抽屉、环境检查、一键批量翻译与长任务恢复、写回安全保护与问题处理折叠、设置上下文主开关单源、空状态 CTA、resume 门控、诊断 splitter 空闲恢复、主题、批量上下文预建与高级诊断页。
...
**真实项目烟测记录**：2026-06-19（早于 2026-07 的 P0–P3 IA 重构）使用一部约 3,300 待译行的真实 Ren'Py 项目跑通 GUI **批量翻译**主路径：预处理、原文索引预建、`build -> submit -> status`、排队后继续、`download -> check`、可写回状态下写回和游戏启动验证。该批次影响 6 个文件、约 3,300 处译文行；首轮检查为「需处理」时 GUI 正确禁用写回，问题经 CLI / 人工修正并重新检查为「可写回」后再写回。该记录证明批量翻译工作流可端到端跑通，但不替代当前 IA 五个任务入口的人工往返烟测。

**#85 / #83 / #84 / #139 验证口径**：同步翻译、关键词提取、订正、翻译 A/B 对比已在 GUI 实现并有单元测试覆盖，但**尚未**各自完成与上述同等级别的真实项目烟测；使用前建议在副本项目上小范围试跑。

**有意延后**：安装包、更完整的多游戏端到端手测矩阵，以及按 [#176](https://github.com/hu-border-collie/renpy-translation-lab/issues/176) 推进的真实页面化与 `app.py` 解耦。
