...
- **任务类型 + 子任务**（两列下拉，联动）：
  ...
  - **分析与准备**：关键词提取（#83 占位）、预建 RAG 库、预建原文索引
  - **维护**：订正（#84 占位）
...
- 还没有完整多项目管理。
- 还没有完整可视化 diff 编辑器。
- retry 目前提供诊断页命令参考和写回页跳转入口；repair / revision 仍作为独立高级 CLI 流程，不包装成同一个 `check=warn` 解锁路径。
...
**任务选择框架（进行中）**：工作台已提供「任务类型 + 子任务」联动选择与按子任务切换的 UI 外壳；Batch 翻译、同步翻译与预建库已可用，关键词与订正待 #83 / #84 接入各自 workflow。

**有意延后或拆 follow-up**：关键词提取模式（#83）、完整订正模式（#84）、retry 图形化编排、repair 独立 GUI 模式、安装包、多项目管理、更完整的多游戏端到端手测矩阵等。
