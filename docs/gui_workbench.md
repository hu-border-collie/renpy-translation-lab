...
| **诊断与工具** | 任务上下文、命令参考、任务记录 + 原始日志；**翻译 A/B 对比**等从这里启动并放大本页日志。试跑 / 拆分见工作台「批量翻译 · 执行 · 高级工具」。 |
...
- **试跑 / 拆分**在工作台 **批量翻译 · 执行 · 高级工具**，输出进工作台日志抽屉。
...
**试跑样本请求（probe）**：在 **批量翻译 · 执行 · 高级工具** 中，若当前有可用的 translation manifest（version 1），可点击「试跑样本请求」对 package 内少量请求做同步 `generate_content` 冒烟测试（默认 `--limit 3`）。高级选项可调整 `--limit`、`--offset` 与 `--api-key-index`。不会提交批量任务、不修改项目文件；摘要可写入任务上下文，原始输出进工作台日志抽屉。适合在 `submit` 前排障 API、模型与请求格式。

**拆分翻译包（split）**：同在 **批量翻译 · 执行 · 高级工具**。若 translation manifest（version 1）含可拆分块，可按上限生成多个子包（默认 `--max-chunks 600`）。高级选项可调整 `--max-items` 与 `--display-name-prefix`。拆分后 RAG 为静态快照，各子包需分别 submit；主按钮行「提交剩余包」用于拆分场景。子包路径会出现在进度/任务上下文，命令参考会补充各子包 `submit` 示例。详见 `docs/context_systems.md`。
...
- 翻译 A/B 对比在「诊断与工具」；拆分 / 试跑在「批量翻译 · 执行 · 高级工具」。
