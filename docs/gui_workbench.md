...
- **CLI 命令**：按当前 manifest 生成可复制的手动命令（`doctor`、`submit`、`status`、`download`、`check`、`apply` 等）；当最近一次 `check=warn` 时，也会补出 `build-retry` / `merge-retry` / `repair` / `build-revisions` / `preview-revisions` / `apply-revisions` 参考命令。
...
- `warn`：禁用写回，引导用户查看问题；写回页提供「补救命令」入口，诊断页命令参考会给出 retry / repair / revision 的最小可复制命令，处理后仍需重新 `check` 到 `safe`。
...
- 还没有完整多项目管理。
- 还没有完整可视化 diff 编辑器。
- repair / retry / revision 目前提供诊断页命令参考和写回页跳转入口；还没有完整图形化编排，执行顺序仍由用户按命令参考推进。
...
**有意延后或拆 follow-up**：同步补译模式（#85）、关键词提取模式（#83）、完整订正模式（#84）、repair/retry/revision 图形化编排、安装包、多项目管理、更完整的多游戏端到端手测矩阵等。
