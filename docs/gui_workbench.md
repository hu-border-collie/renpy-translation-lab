...
在左导航选择 **同步翻译**。页内有备份/直接改文件的风险提示；主按钮为「**开始同步翻译**」。GUI 调用 `gemini_translate.py`（无额外参数），配置仍来自 `translator_config.json` 的 `sync.*` 段。同步模式可能直接改项目文件；结果区仅作说明，不提供批量的 `check/apply` 闸门。Gemini 后端使用现有 Gemini API Key；LiteLLM 后端使用「设置 → LiteLLM」中保存的供应商凭据或 LiteLLM 约定的环境变量，不要求 Gemini API Key。
...
- **安全边界**：同步翻译会直接写入当前项目副本，仍应先备份并小范围验证。Batch 的 `check=safe → apply` 强制门控保持不变，但不覆盖同步模式。
