...
- 完整普通用户发行体验。可选 GUI 已有第一版工作台，但仍是实验性外壳，不是零配置成品（见 `gui_qt/`、`docs/gui_workbench.md` 和 issue #42）
- Excel / HTML 协作流
- 面向普通用户的零配置体验
- 完整的游戏解包 / 打包一体化发布流程
...
- 可选 PySide6 GUI 可以降低常规 Batch 流程的操作成本，但仍复用同一套 CLI、JSON 配置和 `check -> apply` 安全合约。2026-06-19，GUI 基础主路径已在一部约 3,300 待译行的真实 Ren'Py 项目上完成真实项目烟测：原文索引、Batch 提交/继续、下载、检查、安全写回和启动验证跑通；`warn` 结果仍按设计回到 CLI / 人工处理后再重新 `check`。
