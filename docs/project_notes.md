...
- 完整普通用户发行体验。可选 GUI 已有工作台（五页导航：批量 / 同步 / 关键词 / 订正 / 上下文库），批量翻译主路径已完成真实项目烟测；工作区总表可切换多游戏，且批量 RAG/索引开关可按项目保存，但仍缺少打包安装器与更完整端到端 QA，不是零配置成品（见 `gui_qt/`、`docs/gui_workbench.md`）
- Excel / HTML 协作流
- 面向普通用户的零配置体验
- 完整的游戏解包 / 打包一体化发布流程
