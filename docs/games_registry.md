...
- GUI：**设置 → 项目列表 → 选择工作区…**（写入 `translator_config.json` 的 `workspace_root`）
- CLI：`--workspace <path>`，或先在 `translator_config.json` 中设置 `workspace_root`
...
- **选择工作区…**：首次使用必做；路径写入 `translator_config.json` 的 `workspace_root`。未指定前项目列表为空状态，不可扫描/导入/切换
...
- **打开分区时自动扫描新项目**：**默认关闭**；可手动勾选。开启后进入本分区会扫描工作区内未登记的 `Game_*`。偏好写入 `games_registry.json` 的 `preferences`
...
GUI 切换项目 → 环境检查 → 翻译 / 写回
                ↓ 写回后可选同步 GAMES.md
偶尔：**设置 → 项目列表** → 扫描新项目 / 导入游戏 / 快速刷新全部 / 深度刷新单个疑难项
```
