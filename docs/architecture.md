...
> 2026-09-09（#202 Phase A/B/C/D 模型/项目/上下文页）核对：Phase A 文档已合并；Phase B 已落地
...
> `ContextSettingsPage`。其余 6 页与保存收口仍属 Phase D；`MainWindow` 仍持有唯一保存事务、
> dirty 基线与离开保护。
...
- 其余 6 页仍通过 `LegacySettingsPageAdapter` 依赖 `MainWindow` builder 与私有状态；legacy
...
### target（#202 Phase B 最小接线、Phase C LiteLLM 页与 Phase D 模型/项目/上下文页已落地；其余页面属于 D）
